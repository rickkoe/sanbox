...
const ZoneImportPage = () => {
  ...
  useEffect(() => {
    if (activeProjectId && selectedFabric) {
      axios
        .get(`/api/san/aliases/project/${activeProjectId}/`)
        .then((res) => {
  ...
  const convertZonesToTableFormat = (zones) => {
    ...
    const tableData = zones.map((zone) => {
      ...
      zone.members.forEach((wwpn, index) => {
        const matchingAlias = aliasOptions.find(
          (alias) =>
  ...
  const checkForDuplicates = async (zones) => {
    ...
    try {
      ...
      const response = await axios.get(
        `/api/san/zones/project/${activeProjectId}/`
      );
      ...
      zones.forEach((zone) => {
        ...
        const nameMatch = existingZones.find(
          (existing) =>
      ...
      setError(
        "Failed to check for duplicate zones: " +
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      await axios.post("/api/san/zones/save/", payload);
  ...
  const loadSourceProjectZones = async (projectId) => {
    ...
    try {
      const response = await axios.get(`/api/san/zones/project/${projectId}/`);
    ...
    try {
      ...
      const zonesToUpdate = sourceProjectZones
        ...
        .map((zone) => {
          ...
          const members = zone.members_details
            ? zone.members_details.map((member) => ({
      ...
      setSuccess(
        `Successfully added ${selectedZonesToCopy.length} zones to current project!`
      );
...
...`}
              ...
              {unmatchedWWPNs.length > 0 && (
                <Alert variant="warning" className="mb-3">
                  ...
                  <p>
                    The following WWPNs could not be matched to existing
                    aliases. You may need to import these aliases first or edit
                    the member columns manually:
                  </p>
                  <div style={{ maxHeight: "200px", overflowY: "auto" }}>
                    {unmatchedWWPNs.map((unmatched, index) => (
                      ...
                      >
                        <strong>{unmatched.zoneName}</strong> - Member{" "}
                        {unmatched.memberIndex}: {unmatched.wwpn}
                      </div>
                    ))}
                  </div>
                </Alert>
                  ...
                  <div style={{ maxHeight: "200px", overflowY: "auto" }}>
                    {duplicates.map((duplicate, index) => (
                      ...
                      >
                        ...
                        <small className="text-muted">
                          Matches existing zone:{" "}
                          <strong>{duplicate.existingZone.name}</strong>
                        </small>
                      </div>
                    ))}
                  </div>
                    ...
                    <div className="d-flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const tableData = convertZonesToTableFormat(
                            parsedZones.filter(
                              (zone) =>
                                !duplicates.some(
                                  (dup) => dup.name === zone.name
                                )
                            )
                          );
                          setPreviewData([...tableData]);
                  ...
                  <Card.Body style={{ padding: "0" }}>
                      ...
                      <HotTable
                        ...
                        cells={(row, col, prop) => {
                          ...
                          if (
                            col >= 6 &&
                            typeof prop === "string" &&
                            prop.startsWith("member_")
                          ) {
                            const rowData = previewData[row];
                            if (!rowData) return {};

                            const rowFabric = fabricOptions.find(
                              (f) =>
                                f.id.toString() === rowData.fabric?.toString()
                            )?.name;
                            const currentValue = rowData[prop];

                            // Filter aliases for the row's fabric and include_in_zoning
                            const availableAliases = aliasOptions.filter(
                              (alias) =>
                            ...
                            return {
                              ...
                              source: [
                                "",
                                ...availableAliases.map((alias) => alias.name),
                              ],
                            };
                          }
                          return {};
                    ...
                    <div className="p-3 bg-light border-top">
                      <small className="text-muted">
                        ...
                        double-clicking. Use dropdowns to change fabric, zone
                        type, or member assignments. Unmatched WWPNs are marked
                        with "UNMATCHED:" - replace these with valid alias names
                        or leave empty. Right-click for context menu options.
                      </small>
                    </div>
                  </Card.Body>
                </Card>
              ...
              {showPreview &&
                ...
                duplicates.length > 0 && (
                  <Alert variant="info" className="mt-4">
                    ...
                    All parsed zones already exist in the database. No import
                    needed.
                  </Alert>
                )}
            </Card.Body>
          ...
          {sourceProjectZones.length > 0 && (
            <div>
              <div className="d-flex justify-content-between align-items-center mb-2">
                ...
                <div className="d-flex gap-2">
                  <Button
                    ...
                    onClick={() =>
                      setSelectedZonesToCopy(
                        sourceProjectZones.map((zone) => zone.id)
                      )
                    }
                  >
                    Select All
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                        ...
                        <small className="text-muted">
                          ...
                          Type: {zone.zone_type} | Members:{" "}
                          {zone.members_details?.length || 0}
                        </small>
                      </div>
                      <div className="d-flex align-items-center gap-2">
                        ...
                        {zone.exists && (
                          <span className="badge bg-secondary">Exists</span>
                        )}
                        {selectedZonesToCopy.includes(zone.id) && (
                          <svg
                            width="16"
              ...
              <div className="mt-2 text-muted">
                <small>
                  <strong>{selectedZonesToCopy.length}</strong> zones selected
                  for copying
                </small>
              </div>
            </div>
