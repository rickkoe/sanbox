...
const ZoneDeleteScriptsPage = () => {
    ...
    const fetchScripts = async () => {
      try {
        const response = await axios.get(
          `/api/san/zone-deletion-scripts/${config.active_project.id}/?vendor=${sanVendor}`
        );
        ...
        console.error("Error fetching zone deletion scripts:", err);
        setError("Error fetching zone deletion scripts");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCopyToClipboard = () => {
    if (activeTab && scripts[activeTab]) {
      const header = `### ${activeTab.toUpperCase()} ZONE DELETION COMMANDS`;
  ...
  const handleDownloadAllScripts = async () => {
    ...
    Object.entries(scripts).forEach(([fabricName, fabricData]) => {
      ...
      const header = `### ${fabricName.toUpperCase()} ZONE DELETION COMMANDS`;
      const commandsText = Array.isArray(commands) ? commands.join("\n") : "";
    ...
    try {
      ...
      a.href = url;
      a.download = `${config?.active_project?.name || "project"}_zone_deletion_scripts.zip`;
  ...
  return (
    <div className="table-container" style={{ paddingBottom: "50px" }}>
      ...
      {scripts && Object.keys(scripts).length > 0 ? (
        ...
        <Alert variant="info">
          No zone deletion scripts available. Verify the column "Delete" is checked for
          the zones you want to remove.
        </Alert>
        )}
    </div>
  );
...
export default ZoneDeleteScriptsPage;