...
const OldAliasTable = () => {
  ...
  useEffect(() => {
    ...
    if (typeof navigator.block !== "function") {
      console.warn(
        "navigator.block is not supported in this version of react-router."
      );
      return;
    }
  ...
  const fetchFabrics = async (customerId) => {
    try {
      ...
      setFabrics(
        response.data.map((fabric) => ({ id: fabric.id, name: fabric.name }))
      ); // ✅ Ensure ID and Name
    } catch (error) {
  ...
  const handleTableChange = (changes, source) => {
    ...
    changes.forEach(([visualRow, prop, oldValue, newValue]) => {
      if (oldValue !== newValue) {
        ...
        if (
          physicalRow === updatedAliases.length - 1 &&
          newValue.trim() !== ""
        ) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.post(aliasSaveApiUrl, {
  ...
  const confirmDeleteRows = () => {
    ...
    rowsToDelete.forEach((rowIndex) => {
      ...
      if (aliasToDelete && aliasToDelete.id) {
        axios
          .delete(`${aliasDeleteApiUrl}${aliasToDelete.id}/`)
          .then((response) => {
    ...
    const remainingAliases = updatedAliases.filter(
    ...
    <div className="table-container">
        ...
        <>
          <div>
            ...
            <Button
              className="save-button"
              onClick={() =>
                handleNavigationAttempt("/san/aliases/alias-scripts")
              }
            >
          ...
          {saveStatus && (
            <Alert
              variant={saveStatus.includes("Error") ? "danger" : "success"}
              className="mt-2"
            >
              {saveStatus}
            </Alert>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)}>
        ...
        <Modal.Body>
          ...
          alias{rowsToDelete.length > 1 ? "es" : ""}?
          <br />
          <strong>
            {rowsToDelete
              .map((idx) => unsavedAliases[idx]?.name)
              .filter((name) => name)
              .join(", ")}
          </strong>
        </Modal.Body>
