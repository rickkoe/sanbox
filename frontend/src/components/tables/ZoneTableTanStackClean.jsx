...
const ZoneTableTanStackClean = () => {
    ...
    const handleSelectAllPages = useCallback(async () => {
        try {
            ...
            const fetchAllUrl = API_ENDPOINTS.zones.includes('?')
                ? `${API_ENDPOINTS.zones}&page_size=10000`
                : `${API_ENDPOINTS.zones}?page_size=10000`;
    ...
    const handleMarkForDeletion = useCallback(async () => {
        ...
        try {
            ...
            console.log('Marking zones for deletion:', selectedIds);

            // Call API to update junction table action to 'delete'
            const promises = selectedIds.map(zoneId =>
                api.post(`${API_URL}/api/core/projects/${activeProjectId}/mark-zone-deletion/`, {
                    zone_id: zoneId,
                    action: 'delete'
                })
            );
        ...
        <div className="modern-table-container">
            ...
            {isReadOnly && canEditInfrastructure && projectFilter === 'all' && (
                <div className="alert alert-warning mb-3" role="alert">
                    <strong>Customer View is read-only.</strong> Switch to Project View to add, edit, or delete zones.
                </div>
            )}
