...
const ZoneTable = () => {
  ...
  const { 
    zones, unsavedZones, fabrics, aliases, loading, error, saveStatus, 
    memberColumns, newColumnsCount, isDirty, showModal, nextPath,
    showDeleteModal, rowsToDelete
  } = state;
  ...
  const fetchZones = useCallback(async (projectId) => {
  ...
  const fetchFabrics = useCallback(async (customerId) => {
    try {
      const response = await axios.get(`${API_ENDPOINTS.fabrics}${customerId}/`);
      dispatch({ 
        type: 'SET_FABRICS', 
        payload: response.data.map(fabric => ({ id: fabric.id, name: fabric.name }))
      });
    } catch (error) {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (invalidZone) {
      dispatch({ 
        ...
        payload: "⚠️ Zone name is required for existing zones. Please restore the original name or remove the zone." 
      });
      return;
    }

    if (!config?.active_project?.id) {
      dispatch({ type: 'SET_SAVE_STATUS', payload: "⚠️ No active project selected!" });
      return;
    }
    ...
    try {
      ...
      dispatch({ type: 'SET_SAVE_STATUS', payload: "Zones saved successfully! ✅" });
      dispatch({ type: 'SET_DIRTY', payload: false });
      fetchZones(config.active_project.id);
    } catch (error) {
      console.error("❌ Error saving zones:", error);
      dispatch({ type: 'SET_SAVE_STATUS', payload: "⚠️ Error saving zones! Please try again." });
    }
  ...
  const confirmDeleteRows = useCallback(() => {
    ...
    rowsToDelete.forEach(rowIndex => {
      ...
      if (zoneToDelete && zoneToDelete.id) {
        axios.delete(`${API_ENDPOINTS.zoneDelete}${zoneToDelete.id}/`)
          .then(response => {
  ...
  const getCellsConfig = useCallback((row, col) => {
  ...
  return (
    <div className="table-container">
      ...
      <Modal show={showModal} onHide={() => dispatch({ type: 'TOGGLE_MODAL', payload: false })}>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => dispatch({ type: 'TOGGLE_MODAL', payload: false })}>
            Stay on this page
          </Button>
      ...
      <Modal show={showDeleteModal} onHide={() => dispatch({ type: 'TOGGLE_DELETE_MODAL', payload: false })}>
        <Modal.Header closeButton>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => dispatch({ type: 'TOGGLE_DELETE_MODAL', payload: false })}>
            Cancel
          </Button>
